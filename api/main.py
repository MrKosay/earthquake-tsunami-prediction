...
async def simulate(input_data: SimulationInput):
    try:
        try:
            ...
            # Add economic loss estimate if tsunami likelihood is high
            if input_data.tsunami_likelihood > 0.3:
                # Simple simulation model based on magnitude and tsunami likelihood
            ...
            # If there was an error with the simulation,
            # fall back to the demo prediction
            print(f"Simulation failed: {str(inner_e)}. Falling back to demo simulation.")
            
            # Create demo input with the same parameters
            ...
            # Modify the response to match simulation parameters
            ...
            # Update demo response economic loss
            if input_data.tsunami_likelihood > 0.3:
                base_loss = 10000 * (10 ** input_data.magnitude) * input_data.tsunami_likelihood
                if base_loss < 500000:
                    risk_category = "low"
...
async def predict_demo(input_data: LocationInput):
    ...
    try:
        ...
        # Create a simple deterministic prediction based on location
        # This is just for demo purposes, not scientifically accurate
        lat_factor = abs(input_data.latitude) / 90  # 0 to 1 based on latitude
        lng_factor = abs(input_data.longitude) / 180  # 0 to 1 based on longitude
        day_of_year = date_time.timetuple().tm_yday / 366  # 0 to 1 based on day of year
        
        # Simple formula for earthquake likelihood
        eq_likelihood = 0.1 + 0.4 * lat_factor + 0.3 * lng_factor + 0.2 * day_of_year
        eq_likelihood = min(0.95, max(0.05, eq_likelihood))  # Limit between 0.05 and 0.95
        
        # If close to Pacific Ring of Fire, increase earthquake likelihood
        pacific_rim = [
            (35, 135),  # Japan
            (38, -122),  # California
            (-33, -70),  # Chile
        ...
        for rim_lat, rim_lng in pacific_rim:
            ...
            if distance < 20:  # Within 20 degrees
                eq_likelihood = min(0.95, eq_likelihood * 1.5)  # Increase likelihood
                break
        
        # Simple tsunami likelihood based on proximity to ocean and earthquake likelihood
        # Higher near coasts
        coast_distance = abs(input_data.longitude) % 10  # Simple proxy for coast distance
        tsunami_likelihood = eq_likelihood * (1 - coast_distance / 10) * 0.8
        tsunami_likelihood = min(0.95, max(0.01, tsunami_likelihood))
        
        # Create simulated magnitude
        magnitude = 4.0 + eq_likelihood * 4.0  # Range from 4.0 to 8.0
        magnitude_ci = [magnitude - 0.5, magnitude + 0.5]
        
        # Create simulated depth
        depth = 5.0 + eq_likelihood * 25.0  # Range from 5.0 to 30.0 km
        depth_ci = [max(0, depth - 5.0), depth + 5.0]
        
        # Create economic loss estimate
